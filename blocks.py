...
class EncoderSplit(Encoder):
    def __init__(self, num_epi, output_size = 256, filter_size = 5, num_blocks =11):
        super(Encoder, self).__init__()
        self.filter_size = filter_size
        ...
        hiddens = [32, 32, 32, 32, 64, 64] \
        + [128] * (num_blocks - 8) \
        + [256, 256]

        hidden_ins = [32, 32, 32, 32, 32, 64, 64] \
                + [128] * (num_blocks - 8) \
                + [256]

        # 2️⃣ 计算一半维度（向下取整）
        hiddens_half = (np.array(hiddens) // 2).tolist()
        hidden_ins_half = (np.array(hidden_ins) // 2).tolist()
...
class ResBlockDilated(nn.Module):
    def __init__(self, kernel_size, hidden=64, dil=1):
        super().__init__()
...
class Decoder(nn.Module):
    def __init__(self, in_channel, hidden = 256, filter_size = 7, num_blocks = 5):
...
class AttnModule(nn.Module):
    def __init__(self, hidden = 128, layers = 8, record_attn = False, inpu_dim = 256):
        ...
        self.record_attn = record_attn
        self.pos_encoder = PositionalEncoding(hidden, dropout = 0.1,max_len = hidden)
