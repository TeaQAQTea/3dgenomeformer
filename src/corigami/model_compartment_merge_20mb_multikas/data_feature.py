...
class HiCFeature(Feature):
    ...
    def get(self, start, window = 20971520, res = 100000):
...
class GenomicFeature(GenomicFeatureSingleThread):
    ...
    def feature_to_npy(self, chr_name, start, end):
        with pbw.open(self.path) as bw_file:
            signals =  bw_file.stats(chr_name, start, end, type='mean',nBins=256,numpy=True)
        return np.array(signals)
...
class diOutput(GenomicFeatureSingleThread):
    ...
    def feature_to_npy(self, chr_name, start, end):
        ...
        # # 计算原始区间的中点
        # mid_point = int(start) + (int(end) - int(start)) // 2
        # # print(start,end)
        # # 计算中间区间的起始和结束点
        # middle_start = max(int(start), mid_point - middle_length // 2)
        # middle_end = min(int(end), mid_point + middle_length // 2)
        with pbw.open(self.path) as bw_file:
            mean_value = bw_file.stats(chr_name, start, end, type='mean',nBins= 256,numpy=True)
...
>>>>>>> backup/old-main
    ...
    def seq_to_npy(self, seq, start, end):
        ...
        seq = seq[start : end]
        chunk_size=81920
        print(len(seq)//chunk_size)
        print(len(seq))
        #计算256个里的每个的含量
        num_a=np.zeros(len(seq)//chunk_size)
        num_t=np.zeros(len(seq)//chunk_size)
        num_c=np.zeros(len(seq)//chunk_size)
        num_g=np.zeros(len(seq)//chunk_size)
        num_n=np.zeros(len(seq)//chunk_size)
        
        for index in range(0,len(seq)-1,chunk_size):
            chunk = seq[index:index + chunk_size]
            num_a[index//chunk_size]=chunk.count('a') / chunk_size
            num_t[index//chunk_size]=chunk.count('t') / chunk_size
            num_c[index//chunk_size]=chunk.count('c') / chunk_size
            num_g[index//chunk_size]=chunk.count('g') / chunk_size
            num_n[index//chunk_size]=chunk.count('n') / chunk_size

        
        np_seq=np.zeros([len(seq)//chunk_size,5])
        np_seq[:,0]=num_a
        np_seq[:,1]=num_t
        np_seq[:,2]=num_c
        np_seq[:,3]=num_g
