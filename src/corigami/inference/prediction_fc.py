...
def single_prediction(output_path, celltype, chr_name, start, model_path, seq_path, ctcf_path, atac_path):
    ...
    pred = infer.prediction(seq_region, ctcf_region, atac_region, 
                                   model_path)
    #取上三角的index
    index = np.triu_indices(256)
    pred_matrix=np.zeros((256,256))
    pred_matrix[index]=pred
    pred_matrix=pred_matrix+pred_matrix.T-np.diag(pred_matrix.diagonal())
    print(pred_matrix.shape)
    plot = plot_utils.MatrixPlot(output_path, pred_matrix, 'prediction', celltype, 
                                 chr_name, start)
    plot.plot()
