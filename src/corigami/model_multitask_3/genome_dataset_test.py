...
class GenomeDataset(Dataset):
    ...
    def __init__(self, celltype_root, 
        ...
        abc_dicts = {'file_name': 'normed_compartment_same.bw', 'norm':None}
        self.di = self.load_di(f'{celltype_root}/genomic_features', di_dicts)
        self.abc =self.load_abc(f'{celltype_root}/genomic_features', abc_dicts)
        ...
        self.chr_data, self.lengths = self.load_chrs(self.chr_names, self.genomic_features, self.di, self.abc)
    ...
    def __getitem__(self, idx):
        ...
        seq, features, mat, di,abc, start, end = self.chr_data[chr_name][chr_idx]
        if chr_name[:3] == 'chr':
        ...
        if self.include_sequence:
            if self.include_genomic_features: # Both
                outputs = seq, features, mat, di,abc, start, end, chr_name, chr_idx
            else: # sequence only  
                outputs = seq, mat, di,abc, start, end, chr_name, chr_idx
        else: 
            if self.include_genomic_features: # features only
                outputs = features, mat, di,abc, start, end, chr_name, chr_idx
    ...
    def load_chrs(self, chr_names, genomic_features, di,abc):
        '''
        ...
        for chr_name in chr_names:
            ...
            chr_data_dict[chr_name] = ChromosomeDataset(self.data_root, chr_name, omit_regions,di,abc, genomic_features, self.use_aug)
    ...
    def load_di(self, root_dir, feat_dicts):
        '''
        Args:
            features: a list of dicts with 
                1. file name
        ...
        '''
        # print(feat_dicts)
        file_name = feat_dicts['file_name']
        file_path = f'{root_dir}/{file_name}'
        norm = feat_dicts['norm']
        feat=data_feature.diOutput(file_path, norm)
        return feat 
    
    def load_abc(self, root_dir, feat_dicts):
        '''
        Args:
            features: a list of dicts with 
                1. file name
        ...
        file_path = f'{root_dir}/{file_name}'
        norm = feat_dicts['norm']
        feat=data_feature.abcOutput(file_path, norm)
        return feat 
