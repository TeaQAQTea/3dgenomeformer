...
class abcDecoder(nn.Module):
    def __init__(self, hidden_dim):
        ...
        self.res_blocks = self.get_res_blocks(num_blocks, 256)
        self.conv1 = nn.Sequential(
            nn.Conv1d(256, 128, 3, 2, 1),  # 使用步幅 2 缩小长度维度
            nn.BatchNorm1d(128),
            nn.ReLU()
        )
        self.conv2 = nn.Sequential(
            nn.Conv1d(128, 64, 3, 2, 1),  # 使用步幅 2 缩小长度维度
            nn.BatchNorm1d(64),
            nn.ReLU()
        )
        self.conv3 = nn.Sequential(
            nn.Conv1d(64, 32, 3, 2, 1),  # 使用步幅 2 缩小长度维度
            nn.BatchNorm1d(32),
            nn.ReLU()
        )
        self.conv4 = nn.Sequential(
            nn.Conv1d(32, 32, 3, 1, 1),  # 使用步幅 2 将长度缩小到 32
            nn.BatchNorm1d(32),
            nn.ReLU()
        )
        self.fc3 = nn.Linear(32, 1)  # 将通道数缩小到 1

    def get_res_blocks(self, num_blocks, hidden_dim):
        blocks = []
    ...
    def forward(self, x):
        ...
        x = self.conv3(x)
        x = self.conv4(x)
        x = x.permute(0, 2, 1)  # 将维度从 (batch_size, channels, length) 转换为 (batch_size, length, channels)
        x = self.fc3(x).squeeze(2)  # 通过全连接层将通道数缩小到 1 并去掉最后一维度
        print(x.shape)
        return x
