...
class TrainModule(pl.LightningModule):
    ...
    def proc_batch(self, batch):
        ...
        di = di.float()
        abc = abc.float()
        return inputs, mat, di,abc
    
    def training_step(self, batch, batch_idx):
        inputs, mat ,di,abc = self.proc_batch(batch)


        outputs_hic, outputs_di,outputs_abc = self(inputs)
        criterion = torch.nn.MSELoss()
        loss_mat = criterion(outputs_hic, mat)

        criterion_di=torch.nn.MSELoss()
        loss_di=criterion_di(outputs_di,di)
        print(outputs_abc.shape,abc.shape)
        criterion_abc=torch.nn.MSELoss()
        loss_abc=criterion_abc(outputs_abc,abc)

        loss=loss_mat+0.1*loss_di+0.5*loss_abc
        metrics = {'train_step_loss': loss,
                   'train_step_loss_mat': loss_mat,
                   'train_step_loss_di': loss_di,
                   'train_step_loss_abc': loss_abc}
        self.log_dict(metrics, batch_size = inputs.shape[0], prog_bar=True)
        return {'loss': loss, 'loss_mat': loss_mat, 'loss_di': loss_di, 'loss_abc': loss_abc}
    ...
    def _shared_eval_step(self, batch, batch_idx):
        ...
        loss=loss_mat+0.1*loss_di+0.5*loss_abc
        return {'loss':loss,'loss_di':loss_di,'loss_mat':loss_mat,'loss_abc':loss_abc}

    # Collect epoch statistics
    def training_epoch_end(self, step_outputs):
        step_outputs_dict = {'loss': [], 'loss_di': [], 'loss_mat': [], 'loss_abc': []}
        for step_output in step_outputs:
            for key in step_outputs_dict.keys():
                step_outputs_dict[key].append(step_output[key].item())
        ...
        metrics = {'train_loss' : ret_metrics['loss'],
                     ...
                     'train_loss_mat' : ret_metrics['loss_mat'],
                        'train_loss_abc' : ret_metrics['loss_abc']
                  }
    ...
    def validation_epoch_end(self, step_outputs):
        ...
        metrics = {'val_loss' : ret_metrics['loss'],
                     ...
                     'val_loss_mat' : ret_metrics['loss_mat'],
                        'val_loss_abc' : ret_metrics['loss_abc']
                  }
        self.log_dict(metrics, prog_bar=True)
    ...
    def _shared_epoch_end(self, step_outputs):
        ...
        loss_abc = torch.tensor(step_outputs['loss_abc']).mean()
        return {'loss' : loss, 'loss_di' : loss_di, 'loss_mat' : loss_mat, 'loss_abc' : loss_abc}
