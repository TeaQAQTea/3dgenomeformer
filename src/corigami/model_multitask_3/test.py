...
class TrainModule(pl.LightningModule):
    ...
    def proc_batch(self, batch):
        seq, features, mat, di,abc, start, end, chr_name, chr_idx = batch
        features = torch.cat([feat.unsqueeze(2) for feat in features], dim = 2)
        inputs = torch.cat([seq, features], dim = 2)
        mat = mat.float()
        di=di.float()
        abc=abc.float()
        return inputs, mat, di,abc, start, end, chr_name, chr_idx
    ...
    def test_step(self, batch, batch_idx):
        ...
        ret_metrics = {'loss':self._shared_eval_step(batch, batch_idx)['loss'],
                       ...
                       'abc_target':self._shared_eval_step(batch, batch_idx)['abc_target'],
                       'abc_predict':self._shared_eval_step(batch, batch_idx)['abc_predict'],}
    ...
    def _shared_eval_step(self, batch, batch_idx):
        inputs, mat,di,abc,start, end, chr_name, chr_idx = self.proc_batch(batch)
        outputs,outputs_di,outputs_abc,attention = self(inputs)
        criterion = torch.nn.MSELoss()
        loss = criterion(outputs, mat)
        loss_di = criterion(outputs_di, di)
        loss=loss+loss_di
        loss_abc = criterion(outputs_abc, abc)
        loss=loss+loss_abc
        ...
        return {'loss' : loss,'target':mat,'di_target':di,'di_predict':outputs_di,'abc_target':abc,'abc_predict':outputs_abc,'predict':outputs,'start':start,'end':end,'chr_name':chr_name,'chr_idx':chr_idx,'attention':attention}
    ...
    def test_epoch_end(self, step_outputs):
        ...
        step_outputs_dict['abc_target']= [out[0]['abc_target'] for out in step_outputs]
        step_outputs_dict['abc_predict'] = [out[0]['abc_predict'] for out in step_outputs]
        ...
        with h5py.File(output_path, 'w') as f:
            ...
            f.create_dataset(data=ret_metrics['abc_target'].detach().cpu().numpy(), name='abc_target')
            f.create_dataset(data=ret_metrics['abc_predict'].detach().cpu().numpy(), name='abc_predict')
    ...
    def _shared_epoch_end(self, step_outputs):
        ...
        abc_target=torch.cat(step_outputs['abc_target'],dim=0)
        abc_predict=torch.cat(step_outputs['abc_predict'],dim=0)
        ...
        return {'loss' : loss,'target':target,'predict':predict,'di_target':di_target,'di_predict':di_predict,'abc_target':abc_target,'abc_predict':abc_predict}
