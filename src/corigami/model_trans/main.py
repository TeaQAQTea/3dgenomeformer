...
class TrainModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        pearsonr = self.get_pearsonr(mat,outputs)
        metrics = {'train_step_loss': loss,'train_step_pearsonr':pearsonr}
        self.log_dict(metrics, batch_size = inputs.shape[0], prog_bar=True)
        return {'loss':loss,'pearsonr':pearsonr.detach()}
...
>>>>>>> main-clean
    ...
    def training_epoch_end(self, step_outputs):
        ...
        metrics = {'train_loss' : ret_metrics['loss'],'train_pearsonr':ret_metrics['pearsonr']
                  }
...
>>>>>>> main-clean
    ...
    def get_pearsonr(self, input, output):
        pearsonr=torch.zeros(input.shape[0])
...
>>>>>>> main-clean
            ...
            pearsonr_batch = torch.mean(pearsonr)
...
>>>>>>> main-clean
    ...
    #     # 计算真实值和预测值之间的变化差异
    #     # 计算真实值和预测值之间的变化差异
    #     dy_true = torch.pow(y_true[:, 1:, :] - y_true[:, :-1, :], 2)
    #     dx_true = torch.pow(y_true[:, :, 1:] - y_true[:, :, :-1], 2)
    #     dy_pred = torch.pow(y_pred[:, 1:, :] - y_pred[:, :-1, :], 2)
    #     dx_pred = torch.pow(y_pred[:, :, 1:] - y_pred[:, :, :-1], 2)
        
    #     dy_diff = torch.pow(dy_true - dy_pred, 2)
    ...
    def get_diagonal(self, input):
        ...
        for i in range(len(input[1])):
            for j in range(len(input[1])):
                if abs(i - j) <= len(input[1]) / 2:
                    diagonal_region_pixels.append(input[0, i, j].item())
