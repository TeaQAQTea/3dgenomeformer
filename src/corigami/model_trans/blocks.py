...
class Decoder(nn.Module):
    def __init__(self, in_channel, hidden = 256, filter_size = 3, num_blocks = 5):
        ...
        self.net = Net()
        self.conv_end = nn.Conv2d(hidden//2, 1, 1)
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(256, 512, 5, padding=2)  # 256x256 -> 256x256
        self.pool = nn.MaxPool2d(2, 2)  # 256x256 -> 128x128
        self.conv2 = nn.Conv2d(6, 16, 5, padding=2)  # 128x128 -> 128x128
        self.pool2 = nn.MaxPool2d(2, 2)  # 128x128 -> 64x64
        self.transconv1 = nn.ConvTranspose2d(512, 256, 5, stride=2, padding=2, output_padding=1)  # 64x64 -> 128x128
        self.transconv2 = nn.ConvTranspose2d(256, 128, 5, stride=2, padding=2, output_padding=1)  # 128x128 -> 256x256

    def forward(self, x):
        x = self.pool(F.relu(self.conv1(x)))  # After conv1: 256x256 -> After pool: 128x128
        # x = self.pool2(F.relu(self.conv2(x)))  # After conv2: 128x128 -> After pool: 64x64
        x = F.relu(self.transconv1(x))  # After transconv1: 64x64 -> 128x128
        x = F.relu(self.transconv2(x))  # After transconv2: 128x128 -> 256x256
        return x
