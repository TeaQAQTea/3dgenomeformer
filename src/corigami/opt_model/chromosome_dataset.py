...
class ChromosomeDataset(Dataset):
    ...
    def __init__(self, celltype_root, chr_name, omit_regions, feature_list, use_aug = True):
        ...
        self.stride = self.bins/2# bins
        self.chr_name = chr_name

        print(f'Loading chromosome {chr_name}...')

        self.celltype_root = celltype_root
        self.seq_path = f'{celltype_root}/../dna_sequence/{chr_name}.fa.gz'
        self.mat_path = f'{celltype_root}/hic_matrix/{chr_name}.npz'
    ...
    def __getitem__(self, idx):
        if not hasattr(self, 'seq'):
            self.seq = data_feature.SequenceFeature(path=self.seq_path)
        if not hasattr(self, 'mat'):
            self.mat = data_feature.HiCFeature(path=self.mat_path)
        # Shift Augmentations
        if self.use_aug: 
