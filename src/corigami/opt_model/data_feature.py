...
class HiCFeature(Feature):
    ...
    def diag_to_mat(self, ori_load, start, end):
        ...
        diag_region_mean=np.mean(diag_region)
        diag_region=np.log10(diag_region/diag_region_mean+1)*100
        print(diag_region)
        return diag_region
...
class SequenceFeature(Feature):
    ...
    def onehot_encode(self, seq):
        ...
        '''
        seq_emb = np.zeros((len(seq), 5), dtype=np.uint8)
        for i, char in enumerate(seq):
            seq_emb[i, "acgtn".find(char)] = 1
        return seq_emb
