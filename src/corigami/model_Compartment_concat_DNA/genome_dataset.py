...
class GenomeDataset(Dataset):
    ...
    def __init__(self, celltype_root, 
        ...
        if mode == 'train':
            ...
            self.chr_names.remove('chrX')
            # self.chr_names =['chr22'] # chrX removed for consistency
        elif mode == 'val':
        ...
        self.chr_data, self.lengths = self.load_chrs(self.chr_names, self.genomic_features, self.di)
    ...
    def __getitem__(self, idx):
        ...
        if self.include_sequence:
            if self.include_genomic_features: # Both
                outputs = seq, features,  di, start, end, chr_name, chr_idx
            else: # sequence only  
                outputs = seq,  di, start, end, chr_name, chr_idx
        else: 
            if self.include_genomic_features: # features only
                outputs = features,  di, start, end, chr_name, chr_idx
    ...
    def load_chrs(self, chr_names, genomic_features, di):
        '''
        ...
        for chr_name in chr_names:
            ...
            chr_data_dict[chr_name] = ChromosomeDataset(self.data_root, chr_name, omit_regions,di, genomic_features, self.use_aug)
