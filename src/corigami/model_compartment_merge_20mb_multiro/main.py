...
def init_parser():
  ...
  parser.add_argument('--ro-seq', dest='ro_seq', nargs='+',default=['ro_minus','ro_plus'])
...
class TrainModule(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        # summary(self.model,input_size=(4, 256, 7))
        # #保存为txt
    ...
    def proc_batch(self, batch):
        ...
        # print(seq.shape,features.shape)
    ...
    # 读取ro_seq目录结构并构建字典
        ro_dicts = {}
        ro_seq_root = args.ro_seq_root  # 假设你传入了ro_seq的根路径

        # 遍历ro-seq目录下的所有子文件夹
        for subdir_name in os.listdir(ro_seq_root):
            subdir_path = os.path.join(ro_seq_root, subdir_name)
            if os.path.isdir(subdir_path):  # 确保是一个子文件夹
                ro_dicts[subdir_name] = {}  # 初始化子文件夹的字典
                # 遍历子文件夹中的所有文件
                for file_name in os.listdir(subdir_path):
                    ...
                    if os.path.isfile(file_path):
                        ...
                        # 读取norm值，假设这是文件内容的一部分
                        norm_value = "None"  
                        # 填充字典结构
                        ro_dicts[subdir_name][feature_name] = {
                            'file_name': file_name,
                            'norm': norm_value
                        }
        genomic_features={}
