...
class GenomeDataset(Dataset):
    ...
    def __init__(self, celltype_root, 
        ...
        insulation_dicts = {'file_name': 'insulation_score4.sort.bw', 'norm':None}
        self.di = self.load_di(f'{celltype_root}/genomic_features', di_dicts)
        self.insulation = self.load_di(f'{celltype_root}/genomic_features', insulation_dicts)
        ...
        self.chr_data, self.lengths = self.load_chrs(self.chr_names, self.genomic_features, self.di,self.insulation)
    ...
    def __getitem__(self, idx):
        ...
        seq, features, mat, di,insulation, start, end = self.chr_data[chr_name][chr_idx]
        if chr_name[:3] == 'chr':
        ...
        if self.include_sequence:
            if self.include_genomic_features: # Both
                outputs = seq, features, mat, di,insulation, start, end, chr_name, chr_idx
            else: # sequence only  
                outputs = seq, mat, di,insulation, start, end, chr_name, chr_idx
        else: 
            if self.include_genomic_features: # features only
                outputs = features, mat, di,insulation, start, end, chr_name, chr_idx
    ...
    def load_chrs(self, chr_names, genomic_features, di,insulation):
        '''
        ...
        for chr_name in chr_names:
            ...
            chr_data_dict[chr_name] = ChromosomeDataset(self.data_root, chr_name, omit_regions,di,insulation, genomic_features, self.use_aug)
