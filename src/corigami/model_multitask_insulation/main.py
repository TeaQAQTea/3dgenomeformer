...
class TrainModule(pl.LightningModule):
    ...
    def proc_batch(self, batch):
        ...
        return inputs, mat, di,insulation
    
    def training_step(self, batch, batch_idx):
        inputs, mat ,di,insulation = self.proc_batch(batch)

        outputs_hic, outputs_di,outputs_insulation = self(inputs)
        criterion = torch.nn.MSELoss()
        loss_hic = criterion(outputs_hic, mat)

        criterion_di=torch.nn.MSELoss()
        loss_di=criterion_di(outputs_di,di)

        insulation_criterion=torch.nn.MSELoss()
        loss_insulation=insulation_criterion(insulation,outputs_insulation)
        loss=loss_hic+self.args.di_weight*loss_di+10*loss_insulation
        metrics = {'train_step_loss': loss,'hic_loss':loss_hic,'di_loss':loss_di,'insulation_loss':loss_insulation}
    ...
    def _shared_eval_step(self, batch, batch_idx):
        ...
        self.log("mat_loss", loss_hic, prog_bar=True)
        self.log("di_loss", loss_di, prog_bar=True)
        self.log("insulation_loss", loss_insulation, prog_bar=True)
        return loss
