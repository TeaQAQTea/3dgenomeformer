...
class ConvTransModel(ConvModel):
    ...
    def __init__(self, num_genomic_features, mid_hidden = 256, record_attn = False):
        ...
        self.decoder = blocks.Decoder(mid_hidden * 2)
        self.diDecoder_insulation = blocks.diDecoder(mid_hidden )
    ...
    def forward(self, x):
        ...
        insulation = self.diDecoder_insulation(feature)

        x = self.diagonalize(feature)
        x = self.decoder(x).squeeze(1)
        # print(x.shape)
        if self.record_attn:
            return x,di,insulation,attn_weights
        else:
            return x,di,insulation
