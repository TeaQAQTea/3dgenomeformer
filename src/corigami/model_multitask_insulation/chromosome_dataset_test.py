...
class ChromosomeDataset(Dataset):
    ...
    '''
    def __init__(self, celltype_root, chr_name, omit_regions, di_file,insulation_file, feature_list, use_aug = True):
        ...
        self.insulation_file = insulation_file
    ...
    def __getitem__(self, idx):
        ...
        seq, features, mat, di,insulation = self.get_data_at_interval(start, end)
        ...
        if self.use_aug:
            ...
            seq, features, mat,di,insulation = self.reverse(seq, features, mat,di,insulation)

        return seq, features, mat, di,insulation, start, end
    ...
    def reverse(self, seq, features, mat,di,insulation, chance = 0.5):
        '''
        ...
        if r_bool < chance:
            ...
            di_r = np.flip(di).copy()
            
            insulation_r = np.flip(insulation).copy()
            ...
            mat_r = mat
            di_r = di
            insulation_r = insulation
        return seq_r, features_r, mat_r, di_r,insulation_r
    ...
    def get_data_at_interval(self, start, end):
        ...
        mat = np.log(mat + 1)
        di = self.di_file.get(self.chr_name, start, end)
        insulation = self.insulation_file.get(self.chr_name, start, end)

        return seq, features, mat, di,insulation
