...
class TrainModule(pl.LightningModule):
    ...
    def proc_batch(self, batch):
        seq, features, mat, di,insulation, start, end, chr_name, chr_idx = batch
        features = torch.cat([feat.unsqueeze(2) for feat in features], dim = 2)
        inputs = torch.cat([seq, features], dim = 2)
        mat = mat.float()
        di=di.float()
        insulation=insulation.float()
        return inputs, mat, di,insulation, start, end, chr_name, chr_idx
    ...
    def test_step(self, batch, batch_idx):
        ...
        ret_metrics = {'loss':self._shared_eval_step(batch, batch_idx)['loss'],
                       ...
                       'insulation_target':self._shared_eval_step(batch, batch_idx)['insulation_target'],
                       'insulation_predict':self._shared_eval_step(batch, batch_idx)['insulation_predict'],}
    ...
    def _shared_eval_step(self, batch, batch_idx):
        inputs, mat,di,insulation,start, end, chr_name, chr_idx = self.proc_batch(batch)
        outputs,outputs_di,outputs_insulation,attention = self(inputs)
        criterion = torch.nn.MSELoss()
        loss = criterion(outputs, mat)
        ...
        return {'loss' : loss,'target':mat,'di_target':di,'di_predict':outputs_di,'predict':outputs,'start':start,'end':end,'chr_name':chr_name,'chr_idx':chr_idx,'attention':attention,'insulation_target':insulation,'insulation_predict':outputs_insulation}
    ...
    def test_epoch_end(self, step_outputs):
        ...
        step_outputs_dict['insulation_target'] = [out[0]['insulation_target'] for out in step_outputs]
        step_outputs_dict['insulation_predict'] = [out[0]['insulation_predict'] for out in step_outputs]
        ...
        with h5py.File(output_path, 'w') as f:
            ...
            f.create_dataset(data=ret_metrics['insulation_target'].detach().cpu().numpy(), name='insulation_target')
            f.create_dataset(data=ret_metrics['insulation_predict'].detach().cpu().numpy(), name='insulation_predict')
    ...
    def _shared_epoch_end(self, step_outputs):
        ...
        insulation_target=torch.cat(step_outputs['insulation_target'],dim=0)
        insulation_predict=torch.cat(step_outputs['insulation_predict'],dim=0)
        ...
        return {'loss' : loss,'target':target,'predict':predict,'di_target':di_target,'di_predict':di_predict,'insulation_target':insulation_target,'insulation_predict':insulation_predict}
