...
class ConvTransModel(ConvModel):
    ...
    def __init__(self, num_genomic_features, mid_hidden = 256, record_attn = False,record_feature = False):
        ...
        # self.convsplit =blocks.convsplit(num_genomic_features, mid_hidden)
        self.encoder = blocks.Encoder(5, output_size = mid_hidden, num_blocks = 12)
        self.attn = blocks.AttnModule(hidden = mid_hidden, record_attn = record_attn)
        self.diDecoder = blocks.diDecoder(mid_hidden,record_feature)
        self.decoder = blocks.Decoder(mid_hidden * 2)
    ...
    def forward(self, x):
        ...
        if self.record_feature:
            di, feature = self.diDecoder(feature)
        else:
        ...
        if self.record_attn:
            if self.record_feature:
                return di, attn_weights, feature
            else:
                return di, attn_weights
        else:
            if self.record_feature:
                return di, feature
            else:
