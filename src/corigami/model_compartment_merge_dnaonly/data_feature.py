...
class GenomicFeature(GenomicFeatureSingleThread):
    ...
    def feature_to_npy(self, chr_name, start, end):
        with pbw.open(self.path) as bw_file:
            values= bw_file.values(chr_name, start, end)
            values = np.array(values)
            values_mean = values.reshape(-1, 8).mean(axis=1)
...
class diOutput(GenomicFeatureSingleThread):
    ...
    def feature_to_npy(self, chr_name, start, end):
        ...
        with pbw.open(self.path) as bw_file:
            ...
            values = np.array(values)
            print(values.shape)
            values_mean = values.reshape(-1, 8*8192).mean(axis=1)
            print(values_mean.shape)
        values_mean = np.nan_to_num(values_mean, 0)
...
class SequenceFeature(Feature):
    ...
    def seq_to_npy(self, seq, start, end):
        ...
        # 将序列字符转为索引
        en_seq = np.array([en_dict[ch] for ch in seq], dtype=int)

        # 创建 one-hot 编码矩阵
        seq_emb = np.eye(5)[en_seq]

        # 分块并计算平均值
        seq_emb_avg = seq_emb.reshape(-1, chunk_size, 5).mean(axis=1)
        return seq_emb_avg
