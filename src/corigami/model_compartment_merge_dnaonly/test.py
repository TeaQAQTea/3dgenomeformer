...
class TrainModule(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        # print(ret_metrics)
        return ret_metrics,start,end,chr_name,chr_idx,attention,features

    def _shared_eval_step(self, batch, batch_idx):
        inputs, di,start, end, chr_name, chr_idx = self.proc_batch(batch)
        outputs_di,attention,features = self(inputs)
        criterion = torch.nn.MSELoss()
        ...
        return {'loss' : loss,'di_target':di,'di_predict':outputs_di,'start':start,'end':end,'chr_name':chr_name,'chr_idx':chr_idx,'attention':attention,'features':features}
    ...
    def test_epoch_end(self, step_outputs):
        ...
        features=[out[6] for out in step_outputs]
        ...
        with h5py.File(output_path, 'w') as f:
            ...
            f.create_dataset(data=features.detach().cpu().numpy(), name='features')
