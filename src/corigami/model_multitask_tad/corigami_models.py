...
class ConvTransModel(ConvModel):
    ...
    def forward(self, x):
        ...
        if self.record_feature:  
            di,tad,out_feature = self.diDecoder(feature)
        else:
            di,tad = self.diDecoder(feature)
        ...
        if self.record_attn:
            if self.record_feature:
                return di,tad, attn_weights,out_feature
            else:
                return di,tad, attn_weights
        else:
            if self.record_feature:
                return di,tad,out_feature
            else:
                return di,tad
