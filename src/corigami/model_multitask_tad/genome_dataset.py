...
class GenomeDataset(Dataset):
    ...
    def __init__(self, celltype_root, 
        ...
        tad_dicts = {'file_name': 'tad_values.bw', 'norm':None}
        self.di = self.load_di(f'{celltype_root}/genomic_features', di_dicts)
        self.tad = self.load_tad(f'{celltype_root}/genomic_features',tad_dicts)
        ...
        self.chr_data, self.lengths = self.load_chrs(self.chr_names, self.genomic_features, self.tad,self.di)
    ...
    def __getitem__(self, idx):
        ...
        seq, features, mat, di, start, end = self.chr_data[chr_name][chr_idx]
        if chr_name[:3] == 'chr':
    ...
    def load_chrs(self, chr_names, genomic_features,tad, di):
        '''
        ...
        for chr_name in chr_names:
            ...
            chr_data_dict[chr_name] = ChromosomeDataset(self.data_root, chr_name, omit_regions,tad,di, genomic_features, self.use_aug)
    ...
    def load_tad(self, root_dir, feat_dicts):
        '''
        Args:
            features: a list of dicts with 
                1. file name
        ...
        file_path = f'{root_dir}/{file_name}'
        norm = feat_dicts['norm']
        feat=data_feature.tadOutput(file_path, norm)
        return feat
