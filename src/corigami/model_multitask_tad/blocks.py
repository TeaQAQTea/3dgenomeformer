...
class diDecoder(nn.Module):
    def __init__(self,hidden_dim,save_features=False):
        ...
        self.conv3=nn.Sequential(
                ...
                nn.BatchNorm1d(32),
                nn.ReLU())  
        self.fc_class = nn.Linear(32, 1)
        
        # Attention mechanism
        self.attention = nn.MultiheadAttention(embed_dim=32, num_heads=8, batch_first=True)

        self.fc3 = nn.Linear(32, 1)  # This outputs di for regression task
    ...
    def forward(self, x):
        ...
        class_out,_ = self.attention(x, x, x)
        class_out = self.fc_class(class_out)  # [4, 256, 1]
        print('class_out', class_out.shape)
        
        # 使用 Sigmoid 激活函数
        class_out = torch.sigmoid(class_out).squeeze(2)  # [4, 256]，去除最后一个维度
        print('class_out', class_out.shape)
        if self.save_features:
            return output,class_out,x
        else:
            return output,class_out
