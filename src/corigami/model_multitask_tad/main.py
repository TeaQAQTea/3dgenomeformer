...
class TrainModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        outputs_di, outputs_hic = self(inputs)

        pos_weight = torch.tensor([37.0])  # 根据类别不平衡比例调整
        pos_weight = pos_weight.to(device=outputs_hic.device)

        # 使用 BCEWithLogitsLoss
        criterion_class = torch.nn.BCEWithLogitsLoss(pos_weight=pos_weight)
        loss_mat = criterion_class(mat,outputs_hic)
    ...
    def _shared_eval_step(self, batch, batch_idx):
        ...
        # 使用 BCEWithLogitsLoss
        criterion_class = torch.nn.BCEWithLogitsLoss(pos_weight=pos_weight)
        print('outputs_hic:',outputs_hic.shape)
        print('mat:',mat.shape)
        loss_mat = criterion_class(mat,outputs_hic)
        criterion_di=torch.nn.MSELoss()
        print(outputs_di.shape)
        print(di.shape)
        loss_di=criterion_di(outputs_di,di)
        # sign_mismatch_di = (outputs_di * di < 0).float()
        # penalty_di = self.args.sign_penalty * sign_mismatch_di.mean()
