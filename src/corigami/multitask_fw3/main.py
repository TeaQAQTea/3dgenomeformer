...
class TrainModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = criterion(outputs_hic, mat)

        criterion_di = torch.nn.MSELoss(reduction='none')  # 保留每个样本的 MSE 值
        mse_loss = criterion_di(outputs_di, di)

        # 检查符号是否一致
        sign_mismatch_di = (outputs_di * di < 0).float()

        # 对于符号不一致的项，将 MSE 损失加权（比如乘以10倍）
        weighted_mse_loss = mse_loss * (1 + 2 * sign_mismatch_di)  # 符号不一致时加权

        # 计算最终的平均损失
        loss_di = weighted_mse_loss.mean()
        loss=loss+self.args.di_weight*loss_di
