...
class GenomicFeature(GenomicFeatureSingleThread):
    ...
    def feature_to_npy(self, chr_name, start, end):
        with pbw.open(self.path) as bw_file:
            signals = bw_file.stats(chr_name, int(start), int(end), type = 'mean',nBins = 256*8192)  
        return np.array(signals)
...
class SequenceFeature(Feature):
    ...
    def seq_to_npy(self, seq, start, end):
        ...
        seq = seq[start : end]
        #计算256个里的每个的含量
        num_a=np.zeros(len(seq)//8)
        num_t=np.zeros(len(seq)//8)
        num_c=np.zeros(len(seq)//8)
        num_g=np.zeros(len(seq)//8)
        num_n=np.zeros(len(seq)//8)
        chunk_size=8192
