...
import corigami_models_dna as corigami_models
...
class TrainModule(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        summary(self.model,input_size=(4, 2097152, 5))
        #保存为txt
        self.args = args
    ...
    def proc_batch(self, batch):
        seq,  mat, di, start, end, chr_name, chr_idx = batch
        # features = torch.cat([feat.unsqueeze(2) for feat in features], dim = 2)
        inputs = seq
        mat = mat.float()
        di=di.float()
        return inputs, mat, di, start, end, chr_name, chr_idx
    ...
    def _shared_eval_step(self, batch, batch_idx):
        ...
        return {'loss' : loss,'target':mat,'di_target':di,'di_predict':outputs_di,'predict':outputs,'start':start,'end':end,'chr_name':chr_name,'chr_idx':chr_idx,'attention':attention,'feature':feature}
    ...
    def test_epoch_end(self, step_outputs):
        ...
        step_outputs_dict['feature'] = [out[0]['feature'] for out in step_outputs]
        ...
        with h5py.File(output_path, 'w') as f:
            ...
            f.create_dataset(data=ret_metrics['feature'].detach().cpu().numpy(), name='feature')
    ...
    def _shared_epoch_end(self, step_outputs):
        ...
        feature=torch.cat(step_outputs['feature'],dim=0)
        ...
        return {'loss' : loss,'target':target,'predict':predict,'di_target':di_target,'di_predict':di_predict,'feature':feature}
    ...
    def get_dataset(self, args, mode):
        ...
        dataset = genome_dataset.GenomeDataset(celltype_root, 
                                ...
                                include_genomic_features = False,chrom=args.chromosome)
    ...
    def get_model(self, args):
        ...
        model = ModelClass(num_genomic_features, mid_hidden = 256,record_attn=True,record_feature=True)
        return model
