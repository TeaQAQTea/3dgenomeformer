...
>>>>>>> main-clean
        ...
        inv_var_di  = torch.exp(-2 * self.log_sigma_di)   # 1/σ^2

        loss = 0.5 * inv_var_mat * loss_mat + self.log_sigma_mat \
            + 0.5 * inv_var_di  * loss_di  + self.log_sigma_di
...
>>>>>>> main-clean
        ...
        # sign_mismatch_di = (outputs_di * di < 0).float()
        # penalty_di = self.args.sign_penalty * sign_mismatch_di.mean()
        inv_var_mat = torch.exp(-2 * self.log_sigma_mat)  # 1/σ_mat^2
        inv_var_di  = torch.exp(-2 * self.log_sigma_di)   # 1/σ_di^2
