...
import os
from blocks import EncoderSplit, AttnModule, diDecoder
import torch.nn as nn
from test_model import GenomeModel
import shap
# 加载模型、数据处理等代码与之前的相同...
...
def init_testing(args):
    ...
    pl_module = GenomeModel(
        ...
        decoder=diDecoder(hidden_dim=256,features=True),
...
def init_parser():
  ...
  # Data and Run Directories
    ...
    parser.add_argument('--do-shap', dest='do_shap', action='store_true',
                            help='Whether to run SHAP analysis')
    ...
    parser.add_argument('--model-path', dest='checkpoint_path', default='checkpoints/epoch=999.ckpt', help='Path to the model checkpoint')
