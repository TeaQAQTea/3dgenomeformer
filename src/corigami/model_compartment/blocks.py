...
class diDecoder(nn.Module):
    def __init__(self, hidden_dim):
        ...
        self.cnn = nn.Sequential(
               nn.Conv1d(hidden_dim, hidden_dim, 3, stride=2, padding=1),  # 增加stride来减小特征图长度
        ...
        self.conv1 = nn.Sequential(
                nn.Conv1d(256, 128, 3, stride=2, padding=1),  # 增加stride=2
                nn.BatchNorm1d(128),
                nn.ReLU(),
                nn.MaxPool1d(kernel_size=2))  # 引入MaxPool1d进一步减小特征图长度
        
        self.conv2 = nn.Sequential(
                nn.Conv1d(128, 64, 3, stride=2, padding=1),  # 增加stride=2
                nn.BatchNorm1d(64),
                nn.ReLU(),
                nn.MaxPool1d(kernel_size=2))  # 再次池化
        
        self.conv3 = nn.Sequential(
                nn.Conv1d(64, 32, 3, stride=2, padding=1),  # 增加stride=2
                nn.BatchNorm1d(32),
                nn.ReLU(),
                nn.MaxPool1d(kernel_size=2))  # 再次池化
