...
class EncoderSplit(Encoder):
    def __init__(self, num_epi, output_size = 256, filter_size = 5, num_blocks = 12):
        ...
        # self.cross_attn = CrossAttention(hiddens[-1],hiddens[-1], hiddens[-1])
        self.conv_end = nn.Conv1d(256, output_size, 1)
    ...
    def forward(self, x):
        ...
        # seq_cross = self.cross_attn(epi, seq, seq)
        # epi_cross = self.cross_attn(seq, epi, epi)
...
class TransformerLayer(torch.nn.TransformerEncoderLayer):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", batch_first = True):
        super(TransformerLayer, self).__init__(d_model, nhead, dim_feedforward, dropout, activation, batch_first)
        self.attn=CrossAttention(d_model, d_model, d_model)

    def forward(self, src, src_mask = None, src_key_padding_mask = None):
        # MHA section
        src_norm = self.norm1(src)
        src_side, attn_weights = self.attn(src_norm, src_norm, src_norm)
...
class TransformerLayer(transformer.TransformerEncoderLayer):
    ...
    def forward(self, src, src_mask = None, src_key_padding_mask = None):
        # MHA section
        src_norm = self.norm1(src)
        src_side, attn_weights = self.cross(src_norm, src_norm, src_norm, 
                                    attn_mask=src_mask,
                                    key_padding_mask=src_key_padding_mask)
>>>>>>> backup/old-main
...
>>>>>>> backup/old-main
        ...
        query = self.query(query_input).view(batch_size, query_len, 1, self.query_dim).transpose(1, 2)
        key = self.key(key_input).view(batch_size, key_len, 1, self.query_dim).transpose(1, 2)
