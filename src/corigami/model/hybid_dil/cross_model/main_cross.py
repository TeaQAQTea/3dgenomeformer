...
class TrainModule(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_feature=self.loss(outputs,mat)
        loss = criterion(outputs, mat)+loss_feature
...
class PerceptualLoss(nn.Module):
    def __init__(self):
        ...
        self.slice = nn.Sequential(*list(vgg.children())[:16])  # 取VGG的前几层
        for param in self.slice.parameters():
            param.requires_grad = False

    def forward(self, x, y):
        x=x.unsqueeze(1)  # 变为 [4, 1, 256, 256]
        x =x.repeat(1, 3, 1, 1) 
        y=y.unsqueeze(1)  # 变为 [4, 1, 256, 256]
        y =y.repeat(1, 3, 1, 1) 
