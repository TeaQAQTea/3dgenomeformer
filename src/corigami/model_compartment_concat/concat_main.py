...
class GenomeModel(pl.LightningModule):
    def __init__(self, encoder, transformer, decoder, num_splits, learning_rate, trainer_max_epochs=500, encoder_weights=None, transformer_weights=None):
        ...
        self.conv1d = nn.Conv1d(in_channels=2560, out_channels=256, kernel_size=1)
    ...
    def forward(self, x):
        ...
        transformed_features = self.transformer(concatenated_features)

        # 转置拼接后的特征，使其适合 Conv1d
        transformed_features = transformed_features[0].permute(0, 2, 1)  # 形状从 (4, 256, 2560) 变为 (4, 2560, 256)

        # 通过卷积层，将通道从 2560 变为 256
        conv_output = self.conv1d(transformed_features)  # 形状为 (4, 256, 256)
    ...
    def get_model(self, args):
        ...
        model = ModelClass(3, mid_hidden=256)
        return model
...
def init_parser():
  ...
  # Data and Run Directories
    ...
    parser.add_argument('--num_splits', dest='num_splits', default=10, type=int, help='Number of splits')
...
def init_training(args):
    ...
    pl_module = GenomeModel(
        encoder=EncoderSplit(num_epi=3, output_size=256),
        transformer=TransformerLayer(d_model=2560, nhead=8),
